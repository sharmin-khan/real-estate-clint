    ...
    <div className="max-w-6xl mx-auto p-4">
      ...
      {offers.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {offers.map((offer) => (
            <div key={offer._id} className=" rounded-lg shadow p-4 dark:bg-green-800/80 flex flex-col">
              <img
              ...
              <p className="text-gray-600 dark:text-gray-300">Location: {offer.propertyLocation}</p>
              <p className="text-gray-600 dark:text-gray-300">Agent: {offer.agentName}</p>
              <p className="text-gray-600 dark:text-gray-300">Offered Amount: {offer.offerAmount} BDT</p>
              <p className="text-gray-600 dark:text-gray-300">Status: <span className="font-bold capitalize">{offer.status}</span></p>
              {offer.status === "accepted" && offer._id && (
                <Link to={`/payment/${offer._id}`} className="mt-2">
                  <button className="w-full bg-green-600 text-white py-2 rounded cursor-pointer hover:bg-green-700">Pay</button>
                </Link>
              )}
              {offer.status === "accepted" && !offer._id && (
